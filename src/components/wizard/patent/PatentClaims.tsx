...
const PatentClaims: React.FC = () => {
  ...
  const handleAddClaim = () => {
    ...
    const parentId = dependsOnMatch ? dependsOnMatch[1] : undefined;

    const newClaimItem: Claim = {
      id: Date.now().toString(),
      text: newClaim,
      type: isDependentClaim ? 'dependent' : 'independent',
      ...(isDependentClaim && parentId && { parentId }),
    };
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-xs bg-secondary px-2 py-1 rounded-full">
                          {claim.type === 'independent' ? 'Independent' : `Dependent on Claim ${claim.parentId}`}
                        </span>
                      </div>
